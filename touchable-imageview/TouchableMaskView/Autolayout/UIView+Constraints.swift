...
extension UIView {
    ...
    func constrainEdges(to otherview: UIView, insets: UIEdgeInsets = .zero, usingSafeArea: Bool = false) -> [NSLayoutConstraint] {
        ...
        constraints.append(
            constrainBottom(to: constrainable, nil, inset: insets.bottom)
        )
    ...
    func constrainEdgesVertically(to otherview: UIView, topInsets: CGFloat = 0, bottomInsets: CGFloat = 0, usingSafeArea: Bool = false) -> [NSLayoutConstraint] {
        ...
        constraints.append(
            constrainBottom(to: constrainable, nil, inset: bottomInsets)
        )
        return constraints
    }
    ...
    func constrainWithinLayoutGuide(of viewController: UIViewController, insets: UIEdgeInsets = .zero) -> [NSLayoutConstraint] {
        ...
        let bottomConstraint = constrainBottom(to: viewController.view, inset: insets.bottom)
    ...
    func constrainBottomToTop(of view: Constrainable, offset: CGFloat = 0, relation: ConstraintRelation = .equal, priority: UILayoutPriority = .required, isActive: Bool = true) -> NSLayoutConstraint {
        prepareForAutolayout()
        return constrainBottom(to: view, view.topAnchor, inset: -offset, relation: relation, priority: priority, isActive: isActive)
    }
    
    @discardableResult
    func constrainBottom(to view: Constrainable, _ anchor: NSLayoutYAxisAnchor? = nil, inset: CGFloat = 0, relation: ConstraintRelation = .equal, priority: UILayoutPriority = .required, isActive: Bool = true) -> NSLayoutConstraint {
        prepareForAutolayout()
        
        switch relation {
        case .equal: return bottomAnchor.constraint(equalTo: anchor ?? view.bottomAnchor, constant: -inset).with(priority).set(active: isActive)
        case .equalOrLess: return bottomAnchor.constraint(lessThanOrEqualTo: anchor ?? view.bottomAnchor, constant: -inset).with(priority).set(active: isActive)
        case .equalOrGreater: return bottomAnchor.constraint(greaterThanOrEqualTo: anchor ?? view.bottomAnchor, constant: -inset).with(priority).set(active: isActive)
        }
    }
